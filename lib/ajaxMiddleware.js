...
module.exports= function (argObj) {
	...
	return function (ctx,next) {
		...
		.promise(function (generatedEvent) {
			...
			if(typeof(argObj.restPath)=="function")
				argObj.restUrl= argObj.restPath(ctx,generatedEvent);
			else
				argObj.restUrl= argObj.restPath;

			if(typeof(argObj.verb)=="function")
				argObj.ajaxVerb= argObj.verb(ctx,generatedEvent);
			else
				argObj.ajaxVerb= argObj.verb;


			if(data) //Only create request when data is defined
				return Promise.all([$.ajax({
					url: argObj.restUrl,
					method: argObj.ajaxVerb.toUpperCase(),
					data: data,
